...
if (isset($_POST['uname']) && isset($_POST['password'])) 
{
	...
	}else
	{
        ...
        $success = false;
        $select_unames = mysqli_query($connection, "SELECT * FROM ADMIN");
        ...
        if($success)
        {
            ...
            header("Location: admin_choice.php?username=".$uname."");
        }
        else
        {
            //if here, username not in database or not matched
            header("Location: admin_login.php?error=Access Denied");
        }
